...
// Pure helper function - safe to define outside component scope
...
export const SmoothInput = ({
  ...
  const measurePrefixWidth = useCallback((text) => {
    const input = inputRef.current;
    const measureSpan = measureRef.current;
  ...
  const updateCaretFromInput = useCallback((target) => {
    const selectionStart = target.selectionStart ?? 0;
    const selectionEnd = target.selectionEnd ?? 0;
    ...
    caretOpacity.set(1);
  }, [caretX, caretOpacity, measurePrefixWidth]);
  ...
  }, [inputValue, updateCaretFromInput]);
  ...
  return (
    <div className={cn("w-full relative py-2 px-3 font-body tactile-input", wrapperClassName)}>
      <div
        ref={containerRef}
      ...
      >
        <input
          ...
          ref={inputRef}
          type={type}
          placeholder={placeholder || "smooth input"}
          className={cn(
            "w-full bg-transparent outline-none placeholder:text-stone-400 col-start-1 col-end-2 row-start-1 row-end-2 text-inherit",
            className,
          )}
          style={style}
          value={inputValue}
        ...
        <span
          ref={measureRef}
          aria-hidden
          className="pointer-events-none invisible absolute top-0 left-0 whitespace-pre font-body text-sm"
        />
        <motion.div
          className="bg-chalkboard-green pointer-events-none col-start-1 col-end-2 row-start-1 row-end-2 h-[1em] w-[2px] self-center"
