// Theme transition animation engine (View Transitions API).
// Ported to plain JS from the Skiper UI concept (createAnimation).
// Original concept: rudrodip / Skiper UI (gxuri.me). Independent rebuild.
...
export const createAnimation = (variant, start = "center", blur = false, url) => {
  ...
  if (variant === "polygon") {
    ...
    return {
      ...
      ::view-transition-old(root), .dark::view-transition-old(root) { animation: none; z-index: -1; }
      .dark::view-transition-new(root) { animation-name: reveal-dark-${start}${blur ? "-blur" : ""}; ${blur ? "filter: blur(2px);" : ""} }
      @keyframes reveal-dark-${start}${blur ? "-blur" : ""} { from { clip-path: ${clipPaths.darkFrom}; } to { clip-path: ${clipPaths.darkTo}; } }
      @keyframes reveal-light-${start}${blur ? "-blur" : ""} { from { clip-path: ${clipPaths.lightFrom}; } to { clip-path: ${clipPaths.lightTo}; } }
      `,
    };
  }
  ...
  if (variant === "circle" && start !== "center") {
    ...
    return {
      ...
      ::view-transition-old(root), .dark::view-transition-old(root) { animation: none; z-index: -1; }
      .dark::view-transition-new(root) { animation-name: reveal-dark-${start}${blur ? "-blur" : ""}; ${blur ? "filter: blur(2px);" : ""} }
      @keyframes reveal-dark-${start}${blur ? "-blur" : ""} { from { clip-path: circle(0% at ${pos}); ${blur ? "filter: blur(8px);" : ""} } ${blur ? "50% { filter: blur(4px); }" : ""} to { clip-path: circle(150.0% at ${pos}); ${blur ? "filter: blur(0px);" : ""} } }
      @keyframes reveal-light-${start}${blur ? "-blur" : ""} { from { clip-path: circle(0% at ${pos}); ${blur ? "filter: blur(8px);" : ""} } ${blur ? "50% { filter: blur(4px); }" : ""} to { clip-path: circle(150.0% at ${pos}); ${blur ? "filter: blur(0px);" : ""} } }
      `,
    };
  }
